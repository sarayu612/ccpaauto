...
public class Thespun_com extends BaseClass {

	String siteName = "https://thespun.com/";
	String param = ConfigurationReader.getProperty("param");
	String environment = System.getenv("environment");
	String version = System.getenv("version");
	String acceptOption = "Ok";
	String rejectAction = "Do Not Sell";
	String pmOption = "California Privacy Rights";
	String pmSaveAndExit = "OK/false";
	String pmReject = "Do Not Sell My Information/false";
	...
	@Test
	public void checkRejectAllFromPM() throws Exception {
		PageWrapper pageWrapper = new PageWrapper(driver);
		test = report.startTest(
				"Check USPString returns `1YYN` with Reject All (opt-out) from Privacy Manager: " + siteName);
		test.assignCategory("Reject All - Privacy Manager");

		System.out.println("Start - checkRejectAllFromPM");
		test.log(LogStatus.INFO, "Launch site : " + siteName +" on " + environment + " with " + version);
		runOnEnvironmentWithVersion(siteName, environment, version);
		...
		test.log(LogStatus.INFO, "Choose '" + pmReject + "' option from Privacy Manager");

		pageWrapper.ccpa_PrivacyManagerPage.chooseButton(pmReject);
		Assert.assertTrue(pageWrapper.ccpa_PrivacyManagerPage.disappearPM(test));
		test.log(LogStatus.INFO, "Refresh/reload the page");
		driver.navigate().refresh();
		Assert.assertFalse(pageWrapper.ccpa_ConsentMessagePage.checkForMessage(test));
